...
Factory.prototype.bindEvents = function () {
  ...
  self.slider.on('update', function (values) {
		var _this = $($(this)[0].target).closest('[data-filter-section-item]');
		if(_this.find('div').is('.noUi-line')) {

		}else{
		  _this.find('.noUi-base').append('<div class="noUi-line"></div>')
    }
    
    $('[data-range-from]').val('от ' + _.head(values));
    $('[data-range-to]').val('до ' + _.tail(values));

		var width = _this.find('.noUi-handle-upper').attr('aria-valuenow') - _this.find('.noUi-handle-lower').attr('aria-valuenow');
		_this.find('.noUi-line').css({
		  'left': _this.find('.noUi-handle-lower').attr('aria-valuenow') + '%',
		  'right': _this.find('.noUi-handle-upper').attr('aria-valuenow') + '%',
		  'width': width + '%'
    });
	});
